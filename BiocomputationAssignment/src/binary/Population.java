...
public class Population {
    ...
    String dataSets = "src/csv/dataset2_pop100_mut0.02_gene10.csv"; // Change file name to get different results
    File f;
    FileWriter fw;
    
    //Data set 1/2 both use this code
    public static final int POPULATION_SIZE = 100; 
    public final static double MUTATION_RATE = 0.02;
    public static final int GENE_SIZE = 10;
    // chnage values to whatever suited 
    
    // 6 for set 1.. 7 for set 2..
    //public static final int RULE_SIZE = 6;
    public static final int RULE_SIZE = 7;
    
    /*
   RuleSize = 6 for DataSet1
   RuleSize = 7 for DataSet2
    */
    ...
    public void readFile() {
        //Scanner scan = new Scanner(Population.class.getResourceAsStream("data1.txt"));
...
//        }
    ...
    public void fitnessFunction() {
        ...
        for (int i = 0; i < population.length; i++) {
            population[i].fitnessFunction(dataset); // finding fitnesses
            if (bestFitness < population[i].getFitness()) { //best fitness
                bestFitness = population[i].getFitness();
            }
            if (worstFitness > population[i].getFitness()) { //worst fitness
    ...
    private void newCSV() { // put data into csv file 
        f = new File(dataSets);
        try {
            if (!f.exists()) {
                f.createNewFile();
